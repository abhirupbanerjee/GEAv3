...
SELECT * FROM (VALUES
    ...
    ('QUICK_LINK_1_LABEL', 'GoG', 'string', 'CONTENT', 'Footer Configuration', 'Quick Link 1 Label', 'Label for first quick link', 'GoG', true, 2),
    ('QUICK_LINK_2_LABEL', 'eServices', 'string', 'CONTENT', 'Footer Configuration', 'Quick Link 2 Label', 'Label for second quick link', 'eServices', true, 4),
    ('QUICK_LINK_3_LABEL', 'Constitution', 'string', 'CONTENT', 'Footer Configuration', 'Quick Link 3 Label', 'Label for third quick link', 'Constitution', true, 6),
    ...
    ('GENERAL_INFO_3_URL', 'https://www.gov.gd/emergency-services', 'url', 'CONTENT', 'Footer Configuration', 'General Info 3 URL', 'URL for third general information link', 'https://www.gov.gd/emergency-services', true, 13),
...
-- Pair each URL with its label: Label (2,4,6) followed by URL (3,5,7)
UPDATE system_settings
SET
    subcategory = 'Footer Configuration',
    sort_order = CASE setting_key
        WHEN 'GOG_URL' THEN 3
        WHEN 'ESERVICES_URL' THEN 5
