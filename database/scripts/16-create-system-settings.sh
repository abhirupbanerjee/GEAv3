...
-- Category: AUTHENTICATION - Citizen Login (Feature 2: Citizen Portal)
-- Includes Twilio SMS configuration, phone region settings, and session management
INSERT INTO system_settings (setting_key, setting_value, setting_type, category, subcategory, display_name, description, is_sensitive, default_value, options, min_value, max_value, is_runtime, sort_order)
SELECT * FROM (VALUES
    -- Enable/Disable Toggle
    ('CITIZEN_LOGIN_ENABLED', 'false', 'boolean', 'AUTHENTICATION', 'Citizen Login', 'Enable Citizen Login', 'Allow citizens to create accounts and login via SMS OTP', false, 'false', NULL, NULL, NULL, true, 20),

    -- Twilio SMS Provider Settings
    ('TWILIO_ACCOUNT_SID', '', 'secret', 'AUTHENTICATION', 'Citizen Login', 'Twilio Account SID', 'Account SID from Twilio Console (starts with AC)', true, '', NULL, NULL, NULL, false, 21),
    ('TWILIO_AUTH_TOKEN', '', 'secret', 'AUTHENTICATION', 'Citizen Login', 'Twilio Auth Token', 'Auth Token from Twilio Console', true, '', NULL, NULL, NULL, false, 22),
    ('TWILIO_VERIFY_SERVICE_SID', '', 'secret', 'AUTHENTICATION', 'Citizen Login', 'Twilio Verify Service SID', 'Verify Service SID (starts with VA) - create in Twilio Verify > Services', true, '', NULL, NULL, NULL, false, 23),
    ...
    ('CITIZEN_ALLOWED_COUNTRIES', '["grenada"]', 'multiselect', 'AUTHENTICATION', 'Citizen Login', 'Allowed Countries', 'Countries whose phone numbers can register. Grenada always available.', false, '["grenada"]',
     '{"groups": [
        ...
        {"label": "Caribbean Islands", "collapsed": true, "options": [
        ...
        {"label": "Other Regions", "collapsed": true, "options": [
          {"value": "usa", "label": "United States (+1)"},
          {"value": "uk", "label": "United Kingdom (+44)"},
          {"value": "canada", "label": "Canada (+1)"}
        ]}
      ]}', NULL, NULL, true, 24),
    ('CITIZEN_CUSTOM_COUNTRY_CODES', '', 'string', 'AUTHENTICATION', 'Citizen Login', 'Custom Country Codes', 'Additional ISD codes (comma-separated, e.g., +91,+49,+33)', false, '', NULL, NULL, NULL, true, 25),

    -- Session & Security Settings
    ('CITIZEN_OTP_EXPIRY_MINUTES', '5', 'number', 'AUTHENTICATION', 'Citizen Login', 'OTP Expiry (minutes)', 'How long OTP codes remain valid', false, '5', NULL, 1, 15, true, 26),
    ('CITIZEN_MAX_OTP_ATTEMPTS', '3', 'number', 'AUTHENTICATION', 'Citizen Login', 'Max OTP Attempts', 'Maximum OTP verification attempts before lockout', false, '3', NULL, 1, 10, true, 27),
    ('CITIZEN_SESSION_HOURS', '24', 'number', 'AUTHENTICATION', 'Citizen Login', 'Session Duration (hours)', 'How long citizen sessions remain active', false, '24', NULL, 1, 168, true, 28),
    ('CITIZEN_DEVICE_TRUST_DAYS', '30', 'number', 'AUTHENTICATION', 'Citizen Login', 'Device Trust (days)', 'How long "remember this device" cookies remain valid', false, '30', NULL, 7, 90, true, 29)
) AS v(setting_key, setting_value, setting_type, category, subcategory, display_name, description, is_sensitive, default_value, options, min_value, max_value, is_runtime, sort_order)
WHERE NOT EXISTS (SELECT 1 FROM system_settings WHERE subcategory = 'Citizen Login' LIMIT 1);
