...
function parsePageContext(content: string): Partial<PageContext> | null {
  ...
  if (audienceMatch) context.audience = audienceMatch[1] as 'public' | 'staff' | 'admin';
...
function extractMultiLineSection(block: string, sectionName: string): string[] | undefined {
...
function inferContextFromRoute(route: string): Partial<PageContext> {
  const parts = route.split('/').filter(Boolean);
  const inferred: Partial<PageContext> = {};
...
function generatePageContexts(): void {
  console.log('\n' + '═'.repeat(70));
