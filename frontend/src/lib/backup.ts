...
export async function restoreBackup(
  ...
  try {
    ...
    // Step 3: Terminate active connections and drop/recreate database
    progress('dropping', 'Dropping existing database...', 50)
    const terminateCmd = `PGPASSWORD="${DB_PASSWORD}" psql -h ${DB_HOST} -U ${DB_USER} -d postgres -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '${DB_NAME}' AND pid <> pg_backend_pid();"`
    try { execSync(terminateCmd, { stdio: 'pipe' }) } catch { /* ignore if no sessions */ }
