...
import type { FeedbackStatsData, FeedbackFilters } from '@/types/analytics'

interface ExportButtonProps {
  statsData: FeedbackStatsData
...
export default function ExportButton({ statsData, filters }: ExportButtonProps) {
  ...
  const exportToCSV = () => {
    ...
    try {
      ...
      rows.push(`Average Satisfaction,${parseFloat(String(statsData.overall.avg_satisfaction || 0)).toFixed(2)}`)
      rows.push(`Average Ease,${parseFloat(String(statsData.overall.avg_ease || 0)).toFixed(2)}`)
      rows.push(`Average Clarity,${parseFloat(String(statsData.overall.avg_clarity || 0)).toFixed(2)}`)
      rows.push(`Average Timeliness,${parseFloat(String(statsData.overall.avg_timeliness || 0)).toFixed(2)}`)
      rows.push(`Average Trust,${parseFloat(String(statsData.overall.avg_trust || 0)).toFixed(2)}`)
      ...
      if (statsData.top_services && statsData.top_services.length > 0) {
        ...
        statsData.top_services.forEach((service: any, index: number) => {
          rows.push(`${index + 1},"${service.service_name}","${service.entity_name}",${service.submission_count},${parseFloat(String(service.avg_satisfaction || 0)).toFixed(2)}`)
        })
        rows.push('')
      }
      ...
      if (statsData.by_channel.length > 0) {
        ...
        statsData.by_channel.forEach((channel: any) => {
          rows.push(`${channel.channel},${channel.count},${parseFloat(String(channel.avg_satisfaction || 0)).toFixed(2)}`)
        })
        rows.push('')
      }
      ...
      if (statsData.by_recipient.length > 0) {
        ...
        statsData.by_recipient.forEach((recipient: any) => {
          rows.push(`${recipient.recipient_group},${recipient.count},${parseFloat(String(recipient.avg_satisfaction || 0)).toFixed(2)}`)
        })
        rows.push('')
      }
      ...
      if (statsData.rating_distribution.length > 0) {
        ...
        statsData.rating_distribution.forEach((rating: any) => {
          rows.push(`${rating.rating} Stars,${rating.count},${parseFloat(String(rating.percentage || 0)).toFixed(2)}%`)
        })
        rows.push('')
      }
      ...
      if (statsData.trend && statsData.trend.length > 0) {
        ...
        statsData.trend.forEach((day: any) => {
          rows.push(`${day.date},${day.submissions},${parseFloat(String(day.avg_satisfaction || 0)).toFixed(2)}`)
        })
        rows.push('')
      }
