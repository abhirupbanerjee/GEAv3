...
const navigationItems: NavigationItem[] = [
  ...
  },
  {
    ...
    children: [
      ...
      { label: 'AI Agents', tabKey: 'AI_AGENTS' },
...
function SidebarContent() {
  ...
  const visibleMenuItems = navigationItems.filter((item) => {
    ...
    // Reporting users see everything admin sees (read-only)
    if (item.requiredRole === 'admin' && ['admin', 'reporting'].includes(session?.user?.roleType as string)) return true
