...
export default function NewServiceRequestPage() {
  ...
  const isReporting = session?.user?.roleType === 'reporting';
  const isStaff = session?.user?.roleType === 'staff';
  const userEntityId = session?.user?.entityId;

  // Redirect admin and reporting users - they should not create requests
  useEffect(() => {
    if (status === 'authenticated' && (isAdmin || isReporting)) {
      alert('Only staff users can create service requests.');
      router.push('/admin/service-requests');
    }
  }, [status, isAdmin, isReporting, router]);
