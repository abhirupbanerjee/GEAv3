...
import { FiSettings, FiSave, FiRefreshCw, FiCheckCircle, FiAlertCircle, FiLock, FiUnlock, FiMail, FiUsers, FiLink, FiSliders, FiFileText, FiExternalLink, FiX, FiUpload, FiTrash2, FiEdit2, FiServer, FiZap, FiDatabase, FiDownload, FiPlay, FiClock, FiHardDrive, FiAlertTriangle, FiSmartphone, FiCpu } from 'react-icons/fi'
...
const CATEGORIES = [
  ...
  { key: 'AI_AGENTS', label: 'AI Agents', icon: FiCpu, description: 'Register and manage AI agents shown on the service request review page' },
...
function SettingsPageContent() {
  ...
  const groupedSettings = activeCategory === 'AI_AGENTS' ? {} : groupBySubcategory(currentCategorySettings, activeCategory)
  const hasPendingChanges = Object.keys(pendingChanges).length > 0

  // Block non-admin/non-reporting access (placed after all hooks to follow React Rules of Hooks)
  if (status !== 'loading' && !['admin', 'reporting'].includes(session?.user?.roleType as string)) {
    return (
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {session?.user?.roleType === 'reporting' ? (
      ...
      <div className="space-y-6">
        ...
        {/* AI Agents - Custom Panel */}
        {activeCategory === 'AI_AGENTS' && (
          <AiAgentsPanel />
        )}

        {/* Empty state */}
        {currentCategorySettings.length === 0 && activeCategory !== 'CONTENT' && activeCategory !== 'SERVICE_PROVIDERS' && activeCategory !== 'USER_MANAGEMENT' && activeCategory !== 'DATABASE' && activeCategory !== 'AI_AGENTS' && (
